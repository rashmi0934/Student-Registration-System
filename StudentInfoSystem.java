...
public class StudentInfoSystem {
    ...
    public List<Subject> sortSubjectsByName() {
        ...
        for (int i=0; i<n-1; i++){
            ...
            for (int j=0; j<n-1; j++){
                Subject currSubject = (Subject) subjectsByName.get(j);
                Subject nextSubject = (Subject) subjectsByName.get(j+1);
                if (currSubject.name.compareTo(nextSubject.name)>0){
                    subjectsByName.set(j, nextSubject);
                    subjectsByName.set(j+1, currSubject);
                    swapped = true;
                }
            }
            if (! swapped){
    ...
    public List<Student> sortStudentsByName() {
        ...
        for (int i = 1; i < n; i++) {
            ...
            int j = i - 1;
            while (j >= 0) {
                Student nextStudent = (Student) students.get(j);
                if (nextStudent.name.compareTo(currStudent.name) > 0) {
                    students.set(j + 1, nextStudent);
                    j--;
                } else {
                    break;
                }
    ...
    public Student findStudent(Integer studentNumber) {
        ...
        while (start<=end){
            int mid = (start+end)/2;
            int midNumber = students.get(mid).studentNumber;
